...
import ptt.client.weapons.shaft.FireTarget
import ptt.client.weapons.shaft.ShotTarget
...
class ShaftWeaponHandler(
  ...
  suspend fun fireArcade(target: FireTarget) {
    ...
    if(target.target != null) {
      ...
      val damageType = DamageType.Normal

      Command(CommandName.DamageTank, target.target, randomDamage.toString(), damageType.key).send(sourceTank)
