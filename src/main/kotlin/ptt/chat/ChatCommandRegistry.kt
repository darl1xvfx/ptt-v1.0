...
class ParsedCommandArguments(
  ...
  operator fun <T : Any> get(name: String): T = getOrNull(name) ?: throw IllegalArgumentException("Отсутствие аргумента с именем $name")
...
class Command(val name: String) {
  ...
  var permissions: Bitfield<Permissions> = Bitfield(0)
