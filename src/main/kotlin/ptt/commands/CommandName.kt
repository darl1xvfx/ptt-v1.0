...
enum class CommandName(val category: CommandCategory, val key: String, val side: CommandSide) {
  ...
  Temperature(CommandCategory.Battle, "change_temperature_tank", CommandSide.Client),

  JuggernautSpawn(CommandCategory.Battle, "juggernaut_spawn", CommandSide.Client), // Звук - текст для спавна Джаггернаута.
  JuggernautDestroyed(CommandCategory.Battle, "juggernaut_destoroyed", CommandSide.Client), // Звук - текст уничтожения - самоунчичтожения для Джаггернаута.
  SoundsOfSupplies(CommandCategory.Battle, "sounds_of_supplies", CommandSide.Client), // Звуки для припасов.
  ...
  СaptchaUpdated(CommandCategory.Lobby, "captchaUpdated", CommandSide.Client),
  ...
  CreateBattleTimeout(CommandCategory.Lobby, "createFailedTooManyBattlesFromYou", CommandSide.Client),
  ...
  ActivateTank(CommandCategory.Battle, "activate_tank", CommandSide.Server),
  ClientActivateTank(CommandCategory.Battle, "activate_tank", CommandSide.Client),
  ...
  DeleteBattle(CommandCategory.Lobby, "remove_battle", CommandSide.Client),
  HideBattle(CommandCategory.Lobby, "hide_battle_info", CommandSide.Client),
  ...
  companion object {
    fun get(category: CommandCategory, key: String, side: CommandSide) = values()
      .singleOrNull { command -> command.category == category && command.key == key && command.side == side }
  }
